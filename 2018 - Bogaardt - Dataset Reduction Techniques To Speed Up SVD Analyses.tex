...
%\Title{Dataset Reduction Techniques to Speed Up Singular Value Decomposition Analyses}
\Title{A review of Singular Values Decomposition techniques and their applications: focus on autocorrelation and big geodatasets}
% A review of Singular Values Decomposition techniques and their applications: high correlate dataset or big geodata.
...
% \textit{Singular Value Decomposition} (SVD) is a mathematical matrix decomposition which has found multiple applications in geosciences. When analysing a spatial field for a single time period, SVD's can help identify structure within the data. In many real-world applications, however, fields include multiple time periods spanning weeks, months or years. And therefore, SVD has become an important tool mainly due to the high redundancy both between temporal acquisitions and between neighbouring pixels of the images. In general, SVD is used in geosciences as a dimensional reduction technique that is very helpful to provide accurate products and reduce the deficiencies of the data \cite{izquierdo17}. The main idea of SVD is to obtain new data representations to avoid problems in the original data (noise, redundancy) and to get simple and accurate final products. For example, Partial Least Square method, which is based on SVD decomposition, is applied in classification \cite{izquierdo14} and regression \cite{hansen03} tasks. In order to analyse multi-temporal data, \textit{Maximum Covariance Analysis} (MCA) and \textit{Canonical Correlation Analysis} (CCA) are more suitable methods \cite{munoz13}. However in the case of filtering out noise, a combination of CCA with minimum noise fraction (MNF/MAF) is useful method \cite{nielsen07}. SVD decomposition is also appropriate in the spatio-temporal analysis such as the comparison of the aerosol optical depth measurements acquired by stations and remote sensing images \cite{li14,li14b}. These analyses rely on the SVD procedure to find patterns which occur frequently and simultaneously~\cite{Eshel2011, Storch1999}. 
% 
% The large amounts of geodata acquire nowadays takes the SVD decomposition to develop new techniques because of performing SVD's on large datasets can be computationally costly and time consuming. Furthermore, real-world data are gathered by machines with finite precision, which means that the \textit{information} contained in the data is limited by the noise level. These real data are represented by a matrix; for instance, a group of~$n$ individuals who report scores on~$m$ questions or the temperatures at~$m$ locations measured over~$n$ time periods. These values can be arranged in a matrix with~$m$ rows and~$n$ columns. Like a vector, a matrix is a combination of basis vectors which indicate direction, each with a coefficient which indicates magnitude. As an extension of the vector, a matrix has two bases, the row- and the column basis, which can also be changed via a rotation. A clever basis to rotate into is one where the product of the first row- and column basis vectors explains as much of the variance in the dataset as possible. Subsequent pairs of basis vectors, known as \textit{modes}, explain as much of the remaining variance as possible while being orthogonal to all previous modes. Such basis vectors are called \textit{Principle Components} (PC's) or \textit{Empirical Orthogonal Functions} (EOF's) and they are found via an SVD of the matrix.
% 
...
% 
% This article examines several SVD procedures which exploit autocorrelation and rank decomposition to analyse data in an efficient manner. We explain which type of problems could be tackled with them and make predictions about the error incurred in the approximations based on the level of autocorrelation of the input data. Though the individual techniques are not novel, % a review is beneficial for domains less familiar with analysing large datasets
...
\end{figure}
 ...
 % Perhaps move Ram/Out-of-core higher and make it fundamental to the paper, perhaps even in the title. For researcher who wish to avoid setting up clusters, running parallel jobs etc.
% \subsection{SVD techniques}
